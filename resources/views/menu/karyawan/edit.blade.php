                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('karyawan.update', $karyawan->id) }}" method="POST"
                            ...
                            <div class="col-lg-6">
                                <div class="col-12 mt-3">
                                    ...
                                    <input type="text" id="nik" name="nik"
                                        value="{{ old('nik') ? old('nik') : $karyawan->nik }}"
                            ...
                            <div class="col-lg-6">
                                <div class="col-12 mt-3">
                                    ...
                                    <input type="text" id="telepon" name="telepon" minlength="9" maxlength="16"
                                        value="{{ old('telepon') ? old('telepon') : $karyawan->telepon }}"
                                ...
                                <fieldset class="col-12 mt-3">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="status" id="status1"
                                            value="A"
                                            {{ old('status') ? (old('status') == 'A' ? 'checked' : '') : ($karyawan->status == 'A' ? 'checked' : '') }}>
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="status" id="status2"
                                            value="N"
                                            {{ old('status') ? (old('status') == 'N' ? 'checked' : '') : ($karyawan->status == 'N' ? 'checked' : '') }}>
                            ...
                            <div class="col-lg-6">
                                <div class="col-12 mt-3">
                                    <label for="mulai" class="form-label">Tanggal Mulai Bekerja <span
                                            class="text-danger">*</span></label>
                                    <input type="date" id="mulai" name="mulai" required
                                        value="{{ old('mulai') ? old('mulai') : $karyawan->mulai }}"
                            ...
                            <div class="col-lg-12">
                                <div class="col-12 mt-3">
                                    ...
                                    <textarea id="alamat" name="alamat" class="form-control @error('alamat') is-invalid @enderror">{{ old('alamat') ? old('alamat') : $karyawan->alamat }}</textarea>
                                    @error('alamat')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    ...
                                    <select name="province_id" id="province_id" required
                                        ...
                                        @foreach ($provinces as $province_id)
                                            <option value="{{ $province_id->id }}"
                                                {{ old('province_id') ? (old('province_id') == $province_id->id ? 'selected' : '') : ($karyawan->province_id == $province_id->id ? 'selected' : '') }}>
                                                {{ $province_id->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="regency_id" id="regency_id" required
                                        ...
                                        @foreach ($regencies as $regency_id)
                                            <option value="{{ $regency_id->id }}"
                                                {{ old('regency_id') ? (old('regency_id') == $regency_id->id ? 'selected' : '') : ($karyawan->regency_id == $regency_id->id ? 'selected' : '') }}>
                                                {{ $regency_id->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="district_id" id="district_id" required
                                        ...
                                        @foreach ($districts as $district_id)
                                            <option value="{{ $district_id->id }}"
                                                {{ old('district_id') ? (old('district_id') == $district_id ? 'selected' : '') : ($karyawan->district_id == $district_id->id ? 'selected' : '') }}>
                                                {{ $district_id->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="village_id" id="village_id" required
                                        ...
                                        @foreach ($villages as $village_id)
                                            <option value="{{ $village_id->id }}"
                                                {{ old('village_id') ? (old('village_id') == $village_id ? 'selected' : '') : ($karyawan->village_id == $village_id->id ? 'selected' : '') }}>
                                                {{ $village_id->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-12">
                                <div class="col-12 mt-3">
                                    ...
                                    <input type="hidden" name="oldKTP" value="{{ $karyawan->ktp }}">
                                    @if ($karyawan->ktp)
                                        <img src="{{ Storage::url($karyawan->ktp) }}" width="500"
                            ...
                            <div class="col-lg-12">
                                <div class="col-12 mt-3">
                                    ...
                                    <textarea id="keterangan" name="keterangan" class="form-control @error('keterangan') is-invalid @enderror">{{ old('keterangan') ? old('keterangan') : $karyawan->keterangan }}</textarea>
                                    @error('keterangan')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                            ...
                            <div class="text-center mt-5">
                                <button type="submit" class="btn btn-primary text-center">Perbarui Karyawan</button>
                            </div>
                        </form>
                    </div>
                </div>
